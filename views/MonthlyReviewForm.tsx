      ...
      <div className="bg-slate-900/90 border border-white/10 rounded-[1.5rem] md:rounded-[2.5rem] w-[96vw] md:w-full max-w-6xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        <div className="p-5 md:p-8 border-b border-white/5 flex justify-between items-center bg-white/5">
          <div>
            ...
            <p className="text-[8px] md:text-[10px] text-indigo-400 uppercase tracking-[0.4em] font-black mt-1">Progress Tracking</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-10 space-y-8 md:space-y-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
            ...
            <section className="bg-white/5 p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-white/5 space-y-4 md:space-y-6">
              <h3 className="font-black text-slate-500 uppercase text-[9px] md:text-[10px] tracking-[0.3em] mb-2 md:mb-4">Metadata</h3>
          ...
          <section className="space-y-3 md:space-y-4">
             <h3 className="font-black text-slate-500 uppercase text-[9px] md:text-[10px] tracking-[0.3em]">Core Focus</h3>
             <textarea 
                ...
                className="w-full p-4 md:p-6 bg-white/5 border border-white/10 rounded-[1.5rem] md:rounded-[2rem] focus:ring-2 focus:ring-indigo-500 outline-none text-xs md:text-sm text-slate-300 placeholder:text-slate-600"
             />
          </section>

          <section className="space-y-4 md:space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 md:gap-4">
              <h3 className="font-black text-slate-500 uppercase text-[9px] md:text-[10px] tracking-[0.3em]">Task Progression</h3>
              {!isViewOnly && (
                <button onClick={addTask} className="w-full sm:w-auto bg-white/5 border border-white/10 text-indigo-400 px-5 md:px-6 py-2 md:py-2.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Add Row</button>
              )}
            </div>
              ...
              <table className="w-full text-left text-[11px] md:text-xs min-w-[800px]">
                <thead className="bg-white/5">
                  <tr>
                    ...
                    <th className="px-5 md:px-6 py-3 md:py-4 font-black text-slate-500 uppercase tracking-widest">Challenges</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {tasks.map((t, idx) => (
                    <tr key={t.id} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="p-2">
                        <textarea value={t.description} onChange={e => updateTask(t.id, 'description', e.target.value)} disabled={isViewOnly} className="w-full bg-transparent border-none focus:ring-0 p-3 md:p-4 text-slate-200 resize-none min-h-[70px] md:min-h-[80px]" placeholder="..." />
                      </td>
                      <td className="p-2">
                        <textarea value={t.expectation} onChange={e => updateTask(t.id, 'expectation', e.target.value)} disabled={isViewOnly} className="w-full bg-transparent border-none focus:ring-0 p-3 md:p-4 text-slate-200 resize-none min-h-[70px] md:min-h-[80px]" placeholder="..." />
                      </td>
                      ...
                      <td className="p-2">
                        <textarea value={t.challenges} onChange={e => updateTask(t.id, 'challenges', e.target.value)} disabled={isViewOnly} className="w-full bg-transparent border-none focus:ring-0 p-3 md:p-4 text-slate-500 italic resize-none min-h-[70px] md:min-h-[80px]" placeholder="..." />
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="p-5 md:p-10 bg-white/5 border-t border-white/5 flex flex-col sm:flex-row justify-end gap-3 md:gap-4">
          {!isViewOnly && (
            <>
              <button onClick={() => handleAction(FormStatus.DRAFT)} className="w-full sm:w-auto px-8 py-3.5 md:px-10 md:py-4 bg-white/5 border border-white/10 text-slate-400 font-black rounded-xl md:rounded-2xl hover:bg-white/10 transition-all text-[9px] md:text-[10px] uppercase tracking-[0.2em]">Save</button>
              <button onClick={() => handleAction(FormStatus.SUBMITTED)} className="w-full sm:w-auto px-8 py-3.5 md:px-10 md:py-4 bg-indigo-600 text-white font-black rounded-xl md:rounded-2xl hover:bg-indigo-500 shadow-2xl transition-all text-[9px] md:text-[10px] uppercase tracking-[0.2em] shimmer-container">Commit</button>
            </>
          )}
          {isViewOnly && (
            <button onClick={onClose} className="w-full sm:w-auto px-10 py-3.5 md:px-12 md:py-4 bg-slate-100 text-slate-900 font-black rounded-xl md:rounded-2xl transition-all text-[9px] md:text-[10px] uppercase tracking-[0.2em]">Close</button>
          )}
        </div>
      </div>
    </div>
